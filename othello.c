...
static ssize_t othello_read(struct file *, char *buf, size_t length, loff_t *offset);
static ssize_t othello_write(struct file *, const char *buf, size_t length, loff_t *offset);
static long othello_command(struct file *file, unsigned int code, unsigned long data);
...
void draw()
{
	...
	for (i = 0; i < CANVAS_HEIGHT; i++)
	{
		for (j = 0; j < CANVAS_WIDTH; j++)
		{
			...
			{
				canvas->canvas[i][j] = get_state_Othello(othello, board_x, board_y) == WHITE ? 'O' : '@';
				continue;
			}
			canvas->canvas[i][j] = ' ';
		}
...
static ssize_t othello_read(struct file *file, char *buf, size_t length, loff_t *offset)
{
	long long i;
	int bytes_read = 0;
	if (*offset > (loff_t)(CANVAS_HEIGHT * CANVAS_WIDTH)) return 0;
	draw();
	for (i = (long long)(*offset); i < (long long)(*offset) + length; i++)
	{
		...
		if (i > (size_t)(CANVAS_HEIGHT * CANVAS_WIDTH)) break;
...
static ssize_t othello_write(struct file *file, const char *buf, size_t length, loff_t *offset)
{
	if (length < 1) 
		return 0;
	if (buf[0] == '\0')
	...
	if (buf[0] == 'O' || buf[0] == '@')
	{
		State stone = (buf[0] == 'O' ? WHITE : BLACK);
		if ((stone == WHITE) == othello->turn)
		{
			board_write(*offset);
		}
	}
